...
- 로컬/미설정 환경에서는 현재 Vercel URL `https://whisky-app-vert.vercel.app`를 fallback으로 유지해 빌드가 깨지지 않게 한다.
...
## 리스크
- 커스텀 도메인을 붙이면 Vercel 환경변수 `NEXT_PUBLIC_SITE_URL` 또는 `SITE_URL`을 실제 canonical origin으로 갱신해야 한다.
